...
def create_point_cloud_mesh(v3d_c_np, k=None, radius=0.01):
    # Randomly sample k points or use all if k is None
    if k is None:
        sampled_points = v3d_c_np
    else:
        sampled_points = v3d_c_np[np.random.choice(v3d_c_np.shape[0], k, replace=False)]

    # Create trimesh with points as small icospheres
    point_cloud_meshes = []
    for point in sampled_points:
        icosphere = trimesh.creation.icosphere(radius=radius, subdivisions=1)
        icosphere.apply_translation(point)
        point_cloud_meshes.append(icosphere)

    # Concatenate the individual icospheres into a single mesh
    point_cloud_mesh = trimesh.util.concatenate(point_cloud_meshes)

    return point_cloud_mesh
