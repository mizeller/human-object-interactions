...
materials = {
    ...
    "white-mat": Material(
        color=(1.0, 1.0, 1.0, 1.0), diffuse=0.5, ambient=0.2, specular=0.0
    ),
    "red": Material(color=(0.969, 0.106, 0.059, 1.0), ambient=0.2),
    "red-washed": Material(color=(0.969, 0.106, 0.059, 0.5), ambient=0.2),
...
class HOLDViewer:
    def __init__(
        self,
        render_types=["video"],  # , "rgb", "depth", "mask"],
        interactive=True,
        size=(2024, 2024),
    ):
        if not interactive:
            try:
                ...
                """
                This try-except statement is necessary, because the current HeadlessRenderer requires 
                a display (OpenGL). Spawn a virtual display when working viw SSH.
                (Sometimes the renders looked quite bad, but it works for working remotely. Run experiments
                over AnyDesk on the local machine.) 
                
                Also, for some reason, these errors sometimes occured: `moderngl: this program belongs to a different context`.
                """
                import subprocess
                import time
                import os

                subprocess.Popen(["Xvfb", ":99", "-screen", "0", f"{size[0]}x{size[1]}x24"])
                time.sleep(2)
    ...
    def view_fn_headless(self, num_iter, out_folder):
        ...
        if "video" in self.render_types:
            ...
            logger.info(f"Rendering video to {vid_p}")
