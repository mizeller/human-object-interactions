...
class xdict(dict):
    ...
    """
    Access values like properties (similar to JS objects).
    Source for __getattr__, __setattr__, __deepcopy__ methods:
    https://github.com/BstLabs/py-jdict/blob/main/src/jdict/__init__.py
    """   
    ...
    def __deepcopy__(self, memo): # -> xdict:
        copy = xdict((k, copy.deepcopy(v, memo)) for k, v in self.items())
        memo[id(self)] = copy
        return copy    
